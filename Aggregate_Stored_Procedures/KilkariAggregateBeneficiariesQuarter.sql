CREATE DEFINER=`etluser`@`%` PROCEDURE `KilkariAggregateBeneficiariesQuarter`()
BEGIN
DECLARE dt DATETIME;
...
count(distinct case when s.subscription_status = 'DEACTIVATED' and s.deactivation_reason = 'DEACTIVATED_BY_USER' and s.enddate >= dt and s.enddate < DATE_ADD(dt, INTERVAL 1 QUARTER) then b.id end) as self_deactivated,
count(distinct case when s.subscription_status = 'DEACTIVATED' and s.deactivation_reason in ('MISCARRIAGE_OR_ABORTION', 'MCTS_UPDATE', 'LIVE_BIRTH', 'STILL_BIRTH', 'CHILD_DEATH', 'MATERNAL_DEATH') and s.enddate >= dt and s.enddate < DATE_ADD(dt, INTERVAL 1 QUARTER) then b.id end) as system_deactivation,
count(distinct case when s.subscriptionPack_id = 1 and s.subscription_status = 'COMPLETED' and s.enddate >= dt and s.enddate < DATE_ADD(dt, INTERVAL 1 QUARTER) then b.id end) as mother_completed,
count(distinct case when s.subscriptionPack_id = 2 and s.subscription_status = 'COMPLETED' and s.enddate >= dt and s.enddate < DATE_ADD(dt, INTERVAL 1 QUARTER) then b.id end) as child_completed,
count(distinct case when b.creationdate >= dt and b.creationdate < DATE_ADD(dt, INTERVAL 1 QUARTER) then b.id end) as joined_subscription,
...
count(distinct case when deactivation_reason = 'WEEKLY_CALLS_NOT_ANSWERED' and deactivation_date >= dt and deactivation_date < DATE_ADD(dt, INTERVAL 1 QUARTER) then beneficiaryTracking_id end) as no_answer_deactivation,
count(distinct case when deactivation_reason = 'LOW_LISTENERSHIP' and deactivation_date >= dt and deactivation_date < DATE_ADD(dt, INTERVAL 1 QUARTER) then beneficiaryTracking_id end) as low_listener_deactivation, '0' as subscriptions_rejected
...
SET dt = DATE_ADD(dt, INTERVAL 1 QUARTER);

update ETL_info_table set last_etl_time = dt where table_id = 72;
