...
SET dt = (select cast(last_etl_time as datetime) from ETL_info_table where table_id = 66);
...
count(distinct case when s.subscription_status = 'DEACTIVATED' and s.deactivation_reason = 'DEACTIVATED_BY_USER' and s.enddate >= dt and s.enddate < DATE_ADD(dt, INTERVAL 1 MONTH) then b.id end) as self_deactivated,
count(distinct case when s.subscription_status = 'DEACTIVATED' and s.deactivation_reason in ('MISCARRIAGE_OR_ABORTION', 'MCTS_UPDATE', 'LIVE_BIRTH', 'STILL_BIRTH', 'CHILD_DEATH', 'MATERNAL_DEATH') and s.enddate >= dt and s.enddate < DATE_ADD(dt, INTERVAL 1 MONTH) then b.id end) as system_deactivation,
count(distinct case when s.subscriptionPack_id = 1 and s.subscription_status = 'COMPLETED' and s.enddate >= dt and s.enddate < DATE_ADD(dt, INTERVAL 1 MONTH) then b.id end) as mother_completed,
count(distinct case when s.subscriptionPack_id = 2 and s.subscription_status = 'COMPLETED' and s.enddate >= dt and s.enddate < DATE_ADD(dt, INTERVAL 1 MONTH) then b.id end) as child_completed,
count(distinct case when b.creationdate >= dt and b.creationdate < DATE_ADD(dt, INTERVAL 1 MONTH) then b.id end) as joined_subscription,
...
count(distinct case when deactivation_reason = 'WEEKLY_CALLS_NOT_ANSWERED' and deactivation_date >= dt and deactivation_date < DATE_ADD(dt, INTERVAL 1 MONTH) then beneficiaryTracking_id end) as no_answer_deactivation,
count(distinct case when deactivation_reason = 'LOW_LISTENERSHIP' and deactivation_date >= dt and deactivation_date < DATE_ADD(dt, INTERVAL 1 MONTH) then beneficiaryTracking_id end) as low_listener_deactivation, '0' as subscriptions_rejected
...
'0' as no_answer_deactivation,
'0' as low_listener_deactivation, COUNT(case when rejection_reason not in ('INVALID_LMP_DATE', 'ACTIVE_CHILD_PRESENT', 'ABORT_STILLBIRTH_DEATH', 'UPDATED_RECORD_ALREADY_EXISTS') THEN id END) as 'subscriptions_rejected' from mother_import_rejection where creation_date >= DATE_ADD(dt, INTERVAL -1 DAY) and creation_date < dt  and accepted = 0 group by state_id, district_id, health_block_id, subcentre_id

UNION ALL
select state_id, district_id, health_block_id as healthBlock_id, subcentre_id, dt as date, '0' as 'self_deactivated', '0' as 'system_deactivation', '0' as 'mother_completed', '0' as 'child_completed', '0' as 'joined_subscription',
'0' as no_answer_deactivation,
'0' as low_listener_deactivation, COUNT(case when rejection_reason not in ('INVALID_DOB', 'UPDATED_RECORD_ALREADY_EXISTS') THEN id END) as 'subscriptions_rejected' from child_import_rejection where creation_date >= DATE_ADD(dt, INTERVAL -1 DAY) and creation_date < dt and accepted = 0 group by state_id, district_id, health_block_id, subcentre_id) as a group by state_id, district_id, healthBlock_id, healthSubFacility_id;
...
update ETL_info_table set last_etl_time = dt where table_id = 66;
