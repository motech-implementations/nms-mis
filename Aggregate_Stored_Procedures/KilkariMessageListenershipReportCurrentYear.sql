CREATE DEFINER=`etluser`@`%` PROCEDURE `KilkariMessageListenershipReportCurrentYear`()
BEGIN
DECLARE dt DATETIME;
DECLARE i INT;
...
DELETE from agg_kilkari_message_listenership where period_type = 'CURRENT YEAR';
...
insert into agg_kilkari_message_listenership(location_type, location_id, date, answered_atleast_one_call, answered_more_than_75_per, answered_50_to_75_per, answered_25_to_50_per, answered_1_to_25_per, answered_no_calls, total_beneficiaries_called,period_type) select 'STATE' as location_type, state_id as location_id, date as date, SUM(answered_atleast_one_call), SUM(answered_more_than_75_per), SUM(answered_50_to_75_per), SUM(answered_25_to_50_per), SUM(answered_1_to_25_per), SUM(answered_no_calls), SUM(total_beneficiaries_called),'CURRENT YEAR' as period_type from agg_kilkari_message_listenership_year_temp group by state_id

UNION ALL
select 'DISTRICT' as location_type, district_id as location_id, date as date, SUM(answered_atleast_one_call), SUM(answered_more_than_75_per), SUM(answered_50_to_75_per), SUM(answered_25_to_50_per), SUM(answered_1_to_25_per), SUM(answered_no_calls), SUM(total_beneficiaries_called),'CURRENT YEAR' as period_type from agg_kilkari_message_listenership_year_temp group by district_id

UNION ALL
select 'BLOCK' as location_type, block_id as location_id, date as date, SUM(answered_atleast_one_call), SUM(answered_more_than_75_per), SUM(answered_50_to_75_per), SUM(answered_25_to_50_per), SUM(answered_1_to_25_per), SUM(answered_no_calls), SUM(total_beneficiaries_called),'CURRENT YEAR' as period_type from agg_kilkari_message_listenership_year_temp group by block_id

UNION ALL
select 'SUBCENTRE' as location_type, subcentre_id as location_id, date as date, SUM(answered_atleast_one_call), SUM(answered_more_than_75_per), SUM(answered_50_to_75_per), SUM(answered_25_to_50_per), SUM(answered_1_to_25_per), SUM(answered_no_calls), SUM(total_beneficiaries_called),'CURRENT YEAR' as period_type from agg_kilkari_message_listenership_year_temp group by subcentre_id;

update ETL_info_table set last_etl_time = currdate where table_id = 97;
