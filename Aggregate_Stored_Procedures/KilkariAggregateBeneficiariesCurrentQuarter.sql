CREATE DEFINER=`etluser`@`%` PROCEDURE `KilkariAggregateBeneficiariesCurrentQuarter`()
BEGIN
DECLARE dt DATETIME;
DECLARE i INT;
DECLARE currdate DATETIME;

SET dt = (select cast(last_etl_time as datetime) from ETL_info_table where table_id = 72);
...
DELETE from agg_aggregate_beneficiaries where period_type = 'CURRENT QUARTER';
...
update ETL_info_table set last_etl_time = currdate where table_id = 69;
