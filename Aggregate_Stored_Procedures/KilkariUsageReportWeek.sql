CREATE DEFINER=`etluser`@`%` PROCEDURE `KilkariUsageReportWeek`()
BEGIN
DECLARE dt DATETIME;
DECLARE curdate DATETIME;

SET dt = (select cast(last_etl_time as datetime) from ETL_info_table where table_id = 115);

SET curdate = (SELECT DATE_ADD(CURDATE(), INTERVAL - WEEKDAY(CURDATE()) DAY));

WHILE dt < curdate DO

truncate table agg_kilkari_usage_temp_week;

insert into agg_kilkari_usage_temp_week (state_id, district_id, block_id, subcentre_id, date,  content_listened_more_than_75_per, content_listened_50_to_75_per, content_listened_25_to_50_per, content_listened_1_to_25_per , called_inbox) 
...
from beneficiary_call_measure as bm  LEFT JOIN message_durations md on bm.campaign_id = md.id  where bm.modificationdate between dt and DATE_ADD(dt, INTERVAL 1 WEEK) and bm.beneficiaryTracking_id is not null group by bm.beneficiaryTracking_id) as a LEFT JOIN beneficiary_tracker b on a.beneficiaryTracking_id=b.id group by b.state_id, b.district_id, b.healthBlock_id, b.healthSubFacility_id;
...
select 'STATE' as location_type, state_id as location_id, date as date, SUM(content_listened_more_than_75_per), SUM(content_listened_50_to_75_per), SUM(content_listened_25_to_50_per), SUM(content_listened_1_to_25_per) , SUM(called_inbox), 'WEEK' as period_type from agg_kilkari_usage_temp_week group by state_id

UNION ALL
select 'DISTRICT' as location_type, district_id as location_id, date as date, SUM(content_listened_more_than_75_per), SUM(content_listened_50_to_75_per), SUM(content_listened_25_to_50_per), SUM(content_listened_1_to_25_per) , SUM(called_inbox), 'WEEK' as period_type from agg_kilkari_usage_temp_week group by district_id

UNION ALL
select 'BLOCK' as location_type, block_id as location_id, date as date, SUM(content_listened_more_than_75_per), SUM(content_listened_50_to_75_per), SUM(content_listened_25_to_50_per), SUM(content_listened_1_to_25_per) , SUM(called_inbox), 'WEEK' as period_type from agg_kilkari_usage_temp_week group by block_id
...
update ETL_info_table set last_etl_time = dt where table_id = 115;
