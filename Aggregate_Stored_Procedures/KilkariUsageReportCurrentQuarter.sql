CREATE DEFINER=`etluser`@`%` PROCEDURE `KilkariUsageReportCurrentQuarter`()
BEGIN
DECLARE dt DATETIME;
DECLARE i INT;
...
DELETE from agg_kilkari_usage where period_type = 'CURRENT QUARTER';
...
select 'STATE' as location_type, state_id as location_id, date as date, SUM(content_listened_more_than_75_per), SUM(content_listened_50_to_75_per), SUM(content_listened_25_to_50_per), SUM(content_listened_1_to_25_per) , SUM(called_inbox),'CURRENT QUARTER' as period_type from agg_kilkari_usage_quarter_temp group by state_id
...
update ETL_info_table set last_etl_time = currdate where table_id = 119;
