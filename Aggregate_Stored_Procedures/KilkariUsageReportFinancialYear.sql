CREATE DEFINER=`etluser`@`%` PROCEDURE `KilkariUsageReportFinancialYear`()
BEGIN
DECLARE dt DATETIME;
DECLARE currdate DATETIME;

SET dt = (select cast(last_etl_time as datetime) from ETL_info_table where table_id = 121);
set currdate = (select concat(year(CURDATE()), '-04-01 00:00:00'));

WHILE dt < currdate DO

truncate table agg_kilkari_usage_financial_year_temp;

insert into agg_kilkari_usage_financial_year_temp (state_id, district_id, block_id, subcentre_id, date,  content_listened_more_than_75_per, content_listened_50_to_75_per, content_listened_25_to_50_per, content_listened_1_to_25_per , called_inbox) 
...
select 'STATE' as location_type, state_id as location_id, date as date, SUM(content_listened_more_than_75_per), SUM(content_listened_50_to_75_per), SUM(content_listened_25_to_50_per), SUM(content_listened_1_to_25_per) , SUM(called_inbox),'FINANCIAL YEAR' as period_type from agg_kilkari_usage_financial_year_temp group by state_id
...
update ETL_info_table set last_etl_time = dt where table_id = 121;
