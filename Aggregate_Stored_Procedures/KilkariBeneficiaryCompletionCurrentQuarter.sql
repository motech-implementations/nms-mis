CREATE DEFINER=`etluser`@`%` PROCEDURE `KilkariBeneficiaryCompletionCurrentQuarter`()
BEGIN
DECLARE dt DATETIME;
DECLARE i INT;
DECLARE currdate DATETIME;

SET dt = (select cast(last_etl_time as datetime) from ETL_info_table where table_id = 82);
...
DELETE from agg_kilkari_beneficiary_completion where period_type = 'CURRENT QUARTER';
...
select 'STATE' as location_type, state_id as location_id, date as date, SUM(total_beneficiaries_completed), SUM(content_listened_more_than_75_per), SUM(content_listened_50_to_75_per), SUM(content_listened_25_to_50_per), SUM(content_listened_1_to_25_per) , ROUND(SUM(total_age)/count(*), 2), 'CURRENT QUARTER' as period_type from agg_kilkari_beneficiary_completion_temp group by state_id
...
update ETL_info_table set last_etl_time = currdate where table_id = 79;
