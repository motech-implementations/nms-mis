CREATE DEFINER=`etluser`@`%` PROCEDURE `KilkariAggregateBeneficiariesYear`()
BEGIN
DECLARE dt DATETIME;
...
count(distinct case when s.subscription_status = 'DEACTIVATED' and s.deactivation_reason in ('MISCARRIAGE_OR_ABORTION', 'MCTS_UPDATE', 'STILL_BIRTH', 'CHILD_DEATH', 'MATERNAL_DEATH') and s.enddate >= dt and s.enddate < DATE_ADD(dt, INTERVAL 1 YEAR) then b.id end) as system_deactivation,
...
update ETL_info_table set last_etl_time = dt where table_id = 70;
