CREATE DEFINER=`etluser`@`%` PROCEDURE `KilkariListeningMatrixReportCurrentFinancialYear`()
BEGIN
DECLARE dt DATETIME;
DECLARE i INT;
...
DELETE from agg_kilkari_listening_matrix where period_type = 'CURRENT FINANCIAL YEAR';
...
update ETL_info_table set last_etl_time = currdate where table_id = 88;
